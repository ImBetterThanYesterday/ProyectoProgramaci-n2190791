...
ax.set_title("Precio Total Por Cada Uno")
ax.legend(loc=1) # upper left corner"""
plt.plot(su['habitaciones'],su['precioUnitario'])
#pie=habitaciones.value_counts().plot(kind='bar')
plt.title('habitaciones',weight='bold',size=14)
plt.show()
"""
for col in su:
    ...
    pie=su[col].value_counts().plot(kind='bar',colors=colors)
