...
driver = webdriver.Chrome('/Users/Admin/Documents/parcialFinalProgramacion/chromedriver') 
...
for i in range (3):  
    ...
    for apt in apts:
        ...
        mtrssin=mtrs.replace("m²","")
        print (mmtrs)
        print(mtrssin)

        oprices= round(aaaprices)

        with open(f'Informacion_proyecto_apartamenntos_bochalema.csv', 'a', encoding='utf-8') as f:
            f.write(link.get_attribute("href")+","+ str(aaprices)+ ","+ str(arooms)+ ","+ str(oprices)+","+ str(mtrssin))
            f.write('\n')
            count = count +1
